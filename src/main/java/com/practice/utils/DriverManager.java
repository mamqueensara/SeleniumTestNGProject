...
public class DriverManager {//DriverManager is  a helper class
	private static ThreadLocal<WebDriver> driver=new ThreadLocal<>();//EACH thread gets its own browser → no sharing, no conflicts
	//Each thread stores its own WebDriver safely.gives each test (thread) its own browser
	...
	public static WebDriver getDriver() {//Creates & returns a browser instance 
		if (driver.get() == null) {//if the driver is already created.If not, it creates a new browser instance.
			WebDriverManager.chromedriver().setup();//Automatically downloads and configures ChromeDriver.--Automatically handles driver
